...
export const ValidationWorkspaceView: React.FC<ValidationWorkspaceViewProps> = ({
  ...
  // 'CREATED' is the backend's own initial status the instant a run is
  // triggered, before the async job even reaches QUEUED.
  const hasActiveRun = datasetRuns.some((r) => r.status === 'CREATED' || r.status === 'QUEUED' || r.status === 'RUNNING');
    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-outline-variant pb-6">
        ...
        {canTriggerValidation && (
          ...
          >
            <span className={`material-symbols-outlined text-lg ${isTriggering || hasActiveRun ? 'animate-spin' : ''}`}>
              {isTriggering || hasActiveRun ? 'sync' : 'play_circle'}
            </span>
            <span>{isTriggering ? 'Starting…' : hasActiveRun ? 'Running…' : 'Run Validation'}</span>
          </button>
        )}
      </div>
      ...
      <div className="space-y-4">
        {datasetRuns.length === 0 ? (
          ...
          datasetRuns.map((run) => {
            ...
            // A COMPLETED run can trivially score 100% with zero rules ever
            // resolved for this dataset (see backend tasks.py) — never let
            // that read the same as a genuine all-pass run in this list.
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    {noApplicableRules && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-secondary-fixed/40 text-on-secondary-fixed">
                  ...
                  <div className="text-right">
                    {noApplicableRules ? (
                      <span className="font-editorial text-base font-extrabold text-outline">
                        No rules evaluated
                      </span>
                    ) : (
                    ...
                    <p className="text-[10px] text-outline uppercase tracking-wider font-semibold">
                      {noApplicableRules ? 'Quality Score N/A' : 'Quality Score'}
                    </p>
                  </div>
                </div>
